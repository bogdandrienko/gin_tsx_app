...
func TripsAnalyzer() TechTripWithKpd {
	db, err := sql.Open("oracle", "oracle://DISPATCHER:disp@172.30.23.16:1521/PITENEW")
	if err != nil {
		ErrorHandler(err)
	}
...
`)
	...
	//ResByTrip(objs[len(objs)-1])

	//Print(objs[len(objs)-2:])
	//for _, item := range objs[len(objs)-2:] {
	//	Print(item)
	//	ResByTrip(item)
	//}

	return TechTripWithKpd{}
}

func ResByTrip(trip *TechTrip) float32 {
	//dictLoad := map[string]map[string]float64{
	//	"001": {
	...
	load := 0.0
	if dictLoad[trip.Worktype] > 0 {
		load = dictLoad[trip.Worktype]
	}
	...
	pathNorm := 60.0
	if dictPath["Speed full"] > 0 {
		pathNorm = dictPath["Speed full"]
	}

	path := float64(trip.Length) / pathNorm * 60
	unload := dictPath["Unload time"]

	returnPathNorm := 60.0
	if dictPath["Speed empty"] > 0 {
		pathNorm = dictPath["Speed empty"]
	}
	returnPath := float64(trip.Length) / returnPathNorm * 60
	wait := dictPath["Wait time"]
