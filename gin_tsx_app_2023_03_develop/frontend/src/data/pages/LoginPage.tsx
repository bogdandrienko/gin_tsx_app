...
export default function Page(): JSX.Element {
  ...
  function Login(event) {
    ...
    if (captchaCheckStore.data) {
      dispatch(slices.user.userLoginStore.action({ form: { ...user } }));
    }
  }

  useEffect(() => {
    if (userLoginStore.data) {
      utils.setCookie("userAccessToken", userLoginStore.data.access);
      utils.setCookie("userRefreshToken", userLoginStore.data.refresh);
      utils.Delay(() => navigate("/tasks"), 1000);
    }
  }, [userLoginStore.data]);
          ...
          <div className="card shadow custom-background-transparent-low m-0 p-0">
            ...
            <div className="card-footer m-0 p-0">
              <components.StatusStore1
                slice={slices.user.userLoginStore}
